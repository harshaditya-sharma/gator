...
const getFeedsFromName = `-- name: GetFeedsFromName :many
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds WHERE name=$1
`

func (q *Queries) GetFeedsFromName(ctx context.Context, name string) ([]Feed, error) {
	rows, err := q.db.QueryContext(ctx, getFeedsFromName, name)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
